...
def get_item_placeholder(item_name):
    if item_name == "Potion":
        ...
        item_name == "Scroll Part 1"
        or item_name == "Scroll Part 2"
        or item_name == "Scroll Part 3"
    ):
...
def check_scroll(player_inventory):
    if (
        "scroll part 1" in player_inventory
        and "scroll part 2" in player_inventory
        and "scroll part 3" in player_inventory
    ):
        print(
        ...
        if room.coords == tuple(player_coords):
            ...
            if room.has_enemy(player_coords) and room.enemy.alive:
                ...
                if room.enemy.name == "the dungeon master" and player_attack == 25:
                    ...
                    room.enemy.name == "the dungeon master"
                    and "the sword of vanquishing" in player_inventory
                ):
                    print(sword_in_inventory)
                elif (
                    room.enemy.name == "the dungeon master"
                    and "the sword of vanquishing" not in player_inventory
                ):
    ...
    for room in dungeon:
        if room.coords == tuple(player_coords):
            if room.has_enemy(player_coords) and room.enemy.alive:
                ...
                player_input = input(
                    f"\n[Battle-menu]: Should I fight against {room.enemy.name}? (y/n) "
                ).lower()
                if player_input == "y":
...
def player_fight(player_char, enemy):
    ...
    while player_char.alive and enemy.alive:
        ...
        time.sleep(0.5)
        print(
            f"{player_char.name} dealt {player_char.attack} damage to {enemy.name}! {enemy.name.capitalize()} has {enemy.health} health left!"
        )
        ...
        time.sleep(0.5)
        print(
            f"{enemy.name} dealt {enemy.attack} damage to {player_char.name}! {player_char.name} has {player_char.health} health left!"
        )
...
def player_menu(player_char, dungeon):
    ...
    while not game_over_flag:
        ...
        if player_input == "exit":
            ...
            look_around(
                dungeon, player_char.coords, player_char.inventory, player_char.attack
            )
